...
public abstract class SharedCyborgSystem : EntitySystem
{
    ...
    private void OnComponentStartup(EntityUid uid, CyborgComponent component, ComponentStartup args)
    {
        ...
        component.BrainSlot = _container.EnsureContainer<ContainerSlot>(uid, CyborgComponent.BrainSlotId);
        component.InstrumentContainer = _container.EnsureContainer<Container>(uid, CyborgComponent.InstrumentContainerName);
    }
    ...
    public bool TryRemoveModule(EntityUid uid,EntityUid used,EntityUid user, CyborgComponent component, ToolComponent tool)
    {
        if (component.ModuleContainer.ContainedEntities.Count == 0)
        {
            if (_net.IsClient && _timing.IsFirstTimePredicted)
        ...
        _tool.UseTool(used, user, uid, 1f, component.ModulesExtractionMethod, new ModuleRemovalFinishedEvent(), toolComponent: tool);
        return true;
    }
    ...
    public bool TryRemoveBattery(EntityUid uid,EntityUid used,EntityUid user, CyborgComponent component, ToolComponent tool)
    {
        if (!component.BatterySlot.ContainedEntity.HasValue)
        {
            if (_net.IsClient && _timing.IsFirstTimePredicted)
        ...
        _tool.UseTool(used, user, uid, 1f, component.ModulesExtractionMethod, new RemoveBatteryEvent(), toolComponent: tool);
        Dirty(component);
        _actionBlocker.UpdateCanMove(uid);
        return true;
    }
