...
public sealed class CyborgSensorComponent : Component
{
    ...
    /// </summary>
    [DataField("server")] public string? ConnectedServer;
    ...
    /// <summary>
    ///     The station this suit sensor belongs to. If it's null the suit didn't spawn on a station and the sensor doesn't
    ///     work.
    /// </summary>
    [DataField("station")] public EntityUid? StationId;

    /// <summary>
    ///     How often does sensor update its owners status (in seconds). Limited by the system update rate.
    /// </summary>
    [DataField("updateRate")] public TimeSpan UpdateRate = TimeSpan.FromSeconds(2f);
}
