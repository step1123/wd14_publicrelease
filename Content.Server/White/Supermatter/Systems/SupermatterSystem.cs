...
namespace Content.Server.White.Supermatter.Systems
{
    ...
    public override void Update(float frameTime)
    {
        ...
        foreach (var (supermatter, xplode,rads) in EntityManager.EntityQuery<SupermatterComponent, ExplosiveComponent, RadiationSourceComponent>())
        {
    ...
    /// </summary>
    private void HandleOutput(EntityUid uid, float frameTime, SupermatterComponent? sMcomponent = null, RadiationSourceComponent? radcomponent = null, Atmos.GasMixture? mixture = null)
    {
        ...
        sMcomponent.Power = Math.Max(sMcomponent.Power - Math.Min(powerReduction * powerlossInhibitor, sMcomponent.Power * 0.83f * powerlossInhibitor), 0f);
    }

    /// <summary>
    /// Handles environmental damage and dispatching damage warning
    /// </summary>
    private void HandleDamage(EntityUid uid, float frameTime, SupermatterComponent? sMcomponent = null, ExplosiveComponent? xplode = null,Atmos.GasMixture? mixture = null)
    {
        ...
        else
        {
			...
			sMcomponent.Damage = (float)Math.Max(sMcomponent.Damage + (Math.Max(Math.Clamp(absorbedTotalMoles / 200, 0.5, 1) * absorbedGas.Temperature - ((Atmospherics.T0C + sMcomponent.HeatPenaltyThreshold)* sMcomponent.DynamicHeatResistance), 0) * sMcomponent.MoleHeatPenalty / 150 ) * sMcomponent.DamageIncreaseMultiplier, 0);
			//Power only starts affecting damage when it is above 5000
			sMcomponent.Damage = Math.Max(sMcomponent.Damage + (Math.Max(sMcomponent.Power - sMcomponent.PowerPenaltyThreshold, 0)/500) * sMcomponent.DamageIncreaseMultiplier, 0);
			//Molar count only starts affecting damage when it is above 1800
			sMcomponent.Damage = Math.Max(sMcomponent.Damage + (Math.Max(absorbedTotalMoles - sMcomponent.MolePenaltyThreshold, 0)/80) * sMcomponent.DamageIncreaseMultiplier, 0);
            ...
            if (absorbedTotalMoles < sMcomponent.MolePenaltyThreshold)
            {
                ...
                sMcomponent.Damage = Math.Max(sMcomponent.Damage + (Math.Min(absorbedGas.Temperature - (Atmospherics.T0C + sMcomponent.HeatPenaltyThreshold), 0) / 150),0);
            }
            ...
            foreach (var ind in _atmosphere.GetAdjacentTileMixtures(xform.GridUid.Value, indices))
            {
                ...
                float factor = integrity switch
                {
                    var x when x < 10 => 0.0005f,
                    var x when x < 25 => 0.0009f,
                    var x when x < 45 => 0.005f,
                    var x when x < 75 => 0.002f,
                    _ => 0f
                };
                sMcomponent.Damage += Math.Clamp((sMcomponent.Power * factor) * sMcomponent.DamageIncreaseMultiplier, 0, sMcomponent.MaxSpaceExposureDamage);
                break;
            }
            sMcomponent.Damage = Math.Min(sMcomponent.DamageArchived + (sMcomponent.DamageHardcap* sMcomponent.ExplosionPoint),sMcomponent.Damage);
        }
        ...
        if(sMcomponent.Damage > sMcomponent.WarningPoint)
        {
            ...
            if (sMcomponent.YellAccumulator >= sMcomponent.YellTimer)
            {
                ...
                {
                    _chat.TrySendInGameICMessage(uid, Loc.GetString("supermatter-safe-alert", ("integrity", integrity.ToString("0.00"))), InGameICChatType.Speak, hideChat: true);
                }
    ...
    /// </summary>
    private void Delamination(EntityUid uid, float frameTime, SupermatterComponent? sMcomponent = null, ExplosiveComponent? xplode = null, Atmos.GasMixture? mixture = null)
    {
        if (!Resolve(uid, ref sMcomponent, ref xplode))
        {
            return;
